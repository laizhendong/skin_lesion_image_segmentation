...
# In[6]:
"""
CUDA_VISIBLE_DEVICES=0 python -W ignore ISBI2017_application.py \
--run 23 \
--arch Nestnet \
--backbone densenet201 \
--init random \
--verbose 1 \
--data /home/x/lzd/UNetPlusPlus-master/data/ISBI2017/10000
"""
...
parser.add_option("--run", dest="run", help="the index of gpu are used", default=2, type="int")
parser.add_option("--arch", dest="arch", help="Nestnet", default=None, type="string")
...
parser.add_option("--data", dest="DATA_DIR", help="data set location", default="data/ISBI2017", type="string")
...
model_path = "/home/x/lzd/UNetPlusPlus-master/trained_weights/ISBI2017/run_"+str(model_path_idx)+"/"
...
class setup_config():
    ...
    def __init__(self, model="", 
                 ...
                 nb_class=None,
                 DATA_DIR = '/home/x/lzd/UNetPlusPlus-master/data/ISBI2017/10000',
                ):
        self.model = model
...
x_train = np.load(os.path.join(config.DATA_DIR, "imgs_train.npy"))
y_train = np.load(os.path.join(config.DATA_DIR, "imgs_mask_train.npy"))
#nb_cases = x_train.shape[0]
#ind_list = [i for i in range(nb_cases)]
#shuffle(ind_list)
#nb_valid = int(nb_cases*0.1)
...
x_train, y_train = np.einsum('ijkl->ijkl', x_train), np.einsum('ijkl->ijkl', y_train)
y_train = np.array(y_train>0, dtype="int")[:,:,:,0:1]
x_valid = np.load(os.path.join(config.DATA_DIR, "imgs_test.npy"))
y_valid = np.load(os.path.join(config.DATA_DIR, "imgs_mask_test.npy"))
x_valid, y_valid = np.einsum('ijkl->ijkl', x_valid), np.einsum('ijkl->ijkl', y_valid)
...
x_test = np.load(os.path.join(config.DATA_DIR, "imgs_test.npy"))
y_test = np.load(os.path.join(config.DATA_DIR, "imgs_mask_test.npy"))
x_test, y_test = np.einsum('ijkl->ijkl', x_test), np.einsum('ijkl->ijkl', y_test)
...
#model.save_weights(os.path.join(model_path, config.exp_name+"single_gpu_model.h5"))
...
JAI = compute_JAI(y_test, p_test)
dsc = compute_dsc(y_test, p_test)
auc = auc(y_test, p_test)
print("\nSetup: {}".format(config.exp_name))
print(">> Testing dataset mdsc = {:.3f}%".format(np.mean(dsc)))
print(">> Testing dataset mJAI = {:.3f}%".format(np.mean(JAI)))
print(">> Testing dataset mDIC = {:.3f}%".format(eva[3]*100.0))
print(">> Testing dataset Sen = {:.3f}%".format(auc[0]))
print(">> Testing dataset Spe = {:.3f}%".format(auc[1]))
print(">> Testing dataset Prec = {:.3f}%".format(auc[2]))
print(">> Testing dataset Acc = {:.3f}%".format(auc[3]))
#saveResult("/home/x/lzd/UNetPlusPlus-master/ISBI2017/predict_image/4",p_test)
#saveResult("/home/x/lzd/UNetPlusPlus-master/ISBI2017/label",y_test)
